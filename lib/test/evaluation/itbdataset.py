...
class ITBDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        ...
        return Sequence(sequence_info['name'], frames, 'itb', ground_truth_rect[init_omit:,:],
                        object_class=sequence_info['object_class'], language=sequence_info['object_class'])
    ...
    def _get_sequence_info_list(self, base_path):
        ...
        for scene in os.listdir(base_path):
            ...
            for video in videos:
                ...
                video_tmp = {"name": video, "path": scene+'/'+video, "startFrame": 1, "endFrame": len(fs), "nz": len(fs[0].split('/')[-1].split('.')[0]), "ext": "jpg",
